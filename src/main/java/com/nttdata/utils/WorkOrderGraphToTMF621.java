...
import com.nttdata.model.tmf621.EquipmentTO;
...
public class WorkOrderGraphToTMF621 {
    ...
    public WorkTicket transform(WorkOrderTO workOrder) {
        ...
        workTicket.setRelatedEntity(relatedEntities(workOrder.getProject(), workOrder.getLocation(),  workOrder.getEquipment()));
    ...
    private WorkTicketStatusType status(WorkOrderStatusTO state) {
        WorkTicketStatusType status = WorkTicketStatusType.valueOf(state.toString());
    ...
    private List<RelatedEntity> relatedEntities(ProjectTO project, LocationTO location,  EquipmentTO equipment) {
        if (project != null || location != null || equipment != null) {
            ...
            if (equipment != null) {
                RelatedEntity relatedEntity = new RelatedEntity();
                relatedEntity.setId(equipment.getId());
                relatedEntity.setName(equipment.getName());
                relatedEntity.setRole("equipment");
                relatedEntity.set_atBaseType("equipment");
                relatedEntities.add(relatedEntity);
            }
    ...
    private List<RelatedParty> relatedParties(UserTO owner, UserTO assigned) {
        if (owner != null || assigned != null) {
            ...
            if (owner != null) {
                ...
                relatedParty.setName(owner.getEmail());
            ...
            if (assigned != null) {
                ...
                relatedParty.setName(assigned.getEmail());
    ...
    private String changeTimeZone(String dateTime) {
        if (dateTime != null) {
            ...
            int millisec = dateTime.indexOf(".");
            String transformedDate = (millisec != -1) ? (dateTime.substring(0, millisec) + "Z") : dateTime;

            DateTimeFormatter df = DateTimeFormat.forPattern("yyyy-MM-dd'T'HH:mm:ss'Z'");
            DateTime dateToChange = df.withOffsetParsed().parseDateTime(transformedDate);
    ...
    private List<CheckList> checkList(List<CheckListCategoryTO> checkListCategories) {
        if (checkListCategories != null && !checkListCategories.isEmpty()) {
            ...
            checkListCategories.forEach(c -> {
                ...
                if (c.getCheckList() != null && !c.getCheckList().isEmpty()) {
                    List<Item> items = new ArrayList<>();
                    c.getCheckList().forEach(i -> {
                        Item item = new Item();
                        item.setId(i.getId());
                        item.setTitle(i.getTitle());
                        item.setType(i.getType().toString());
                        item.setIsMandatory(i.getIsMandatory());
                        item.setHelpText(i.getHelpText());
                        item.setEnumSelectionMode(i.getEnumSelectionMode() != null ? i.getEnumSelectionMode().toString() : null);
                        item.setSelectedEnumValues(i.getSelectedEnumValues());
                        item.setStringValue(i.getStringValue());
                        item.setChecked(i.getChecked());
                        item.setYesNoResponse(i.getYesNoResponse() != null ? i.getYesNoResponse().toString() : null);
                        item.setEnumValues(i.getEnumValues() != null ? i.getEnumValues() : null);
                        if (i.getFiles() != null) {
                            List<FileTO> listFileTO = new ArrayList<>();
                            i.getFiles().forEach(j -> {
                                FileTO fileTO = new FileTO();
                                fileTO.setId(j.getId());
                                fileTO.setFileName(j.getFileName());
                                fileTO.setFileType(j.getFileType());
                                fileTO.setStoreKey(j.getStoreKey());
                                listFileTO.add(fileTO);
                            });
                            item.setFiles(listFileTO);
                        }
                        items.add(item);
                    });
                    cl.setItem(items);
